...
import p3 from '../assets/img/bg-2.jpg';  // Tambahkan foto lainnya
import Cookies from "js-cookie";

const images = [
    p1,
    p2,
    p3, // Tambahkan lebih banyak gambar jika perlu
];
...
const Mapstore = () => {
    ...
    useEffect(() => {
        if (dashboardData.length > 0) {
            // Shuffle images array to randomize the order
            const shuffledImages = [...images].sort(() => Math.random() - 0.5);
            
            // If there are more dashboard items than images, repeat the images
            const imagesForItems = dashboardData.map((_, index) => {
                return shuffledImages[index % shuffledImages.length];  // Use images cyclically if needed
            });
            ...
            <section className="mapstore mt-10 bg-neutral">
                <div className="container mx-auto px-6 py-10 rounded-xl">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 py-5">
                        <div className="py-4">
                            <span className="text-white text-lg">Aplikasi Portal</span>
                    ...
                    <div className="border-b border-gray-600 flex space-x-4 pb-2">
                        {["Dashboards", "Maps", "GeoStories"].map((tab) => {
                            const count = dashboardData.filter((item) => item.category === tab).length;
                            return (
                                <button
                                    key={tab}
                    ...
                    <div className="mt-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                {dashboardData.length > 0 ? (
                                    dashboardData.map((item, index) => (
                                        <div 
                                            key={item.id} 
                                            className="card relative overflow-hidden border border-gray-700 rounded-lg shadow-md bg-gray-900 transition-all duration-300 hover:scale-105 hover:shadow-lg hover:border-gray-500"
                                        >
                                            <figure className="relative">
                                                <img
                                                    className="w-full h-48 object-cover rounded-t-lg transition-all duration-300 hover:brightness-110"
                                                    src={randomImages[index]} // Gambar unik untuk setiap item berdasarkan index
                                                    alt={item.nama || "Image"}
                                                />
                                            ...
                                            <div className="p-4">
                                                ...
                                                <div className="card-actions justify-end mt-4">
                                                    <a
                                                        ...
                                                        className="btn bg-white text-gray-900 font-semibold px-4 py-2 rounded-lg hover:bg-gray-500 hover:text-white transition-all duration-300"
                                                    >
                                                        Lihat
                                                    </a>
                                                </div>
                        ...
                        {activeTab === "GeoStories" && (
                            <div className="text-white text-lg text-center p-10 border border-gray-500 rounded-lg">
                                Konten Geostories akan segera tersedia.
                            </div>
                        )}
                        {activeTab === "Maps" && (
                            <div className="text-white text-lg text-center p-10 border border-gray-500 rounded-lg">
                                Konten Maps akan segera tersedia.
                            </div>
                        )}
                    </div>
                </div>
